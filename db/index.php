...
//Get all databases
$app->map(['GET', 'POST', 'PUT'],'/', function(Request $request, Response $response){
	$db_name = $request->getParam('name');
	$db_name="plantgenie_translation";
    $sql = "SELECT db,species,abbreviation,version,pubmed_id,tax_id,source,url FROM db order by species";
    try{
        $db = new db();
        $db = $db->connect($db_name);
        $stmt = $db->query($sql);
...
//Get database by species name
$app->map(['GET', 'POST', 'PUT'],'/{sp}', function(Request $request, Response $response){
	$db_name = $request->getParam('name');
	$db_name="plantgenie_translation";
	$sp = $request->getAttribute('sp');
    $sql = "SELECT db,species,version,abbreviation FROM db where abbreviation='$sp' OR  db='$sp'";
    try{
        $db = new db();
        $db = $db->connect($db_name);
        $stmt = $db->query($sql);
